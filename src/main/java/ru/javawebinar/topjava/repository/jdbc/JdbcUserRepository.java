...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        Map<Integer, List<UserRole>> userRole = jdbcTemplate.query("SELECT role, user_id FROM user_roles",
                (rs, row) -> new UserRole(Role.valueOf(rs.getString("role")), rs.getInt("user_id")))
                .stream().collect(Collectors.groupingBy(UserRole::getUserId));
        users.forEach(u -> u.setRoles(userRole.get(u.getId())
                .stream()
